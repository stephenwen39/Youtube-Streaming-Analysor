"""這是一個第三方程式，功能是抓取一個Youtube帳號的影片或直播，
原本會連尚未直播的影片都爬下來（也就是待直播的直播間），但尚未直播的影片沒有資料可供分析，
會擾亂我們的分析流程，所以修改成不會抓尚未直播的影片的版本，

為了增進可讀性，不刪原本的第三方註解

@Reference: https://github.com/dermasmid/scrapetube
@Edittor: Chan Wei Wen"""
...
def get_channel(
    ...
    for video in videos:
        if 'lengthText' in video: # 這行是自行改寫的，可以排除尚未直播的影片
            yield video
